...
private extern class CFrameClass
{
	 ...
	 */
	@:native("new")
	static function fromPosition(pos:Vector3):CFrame;

	/**
	 * Returns a CFrame with the position of the first Vector3 and an orientation pointed toward the second.
	 */
	@:native("new")
	static function fromPositionAndOrientation(pos:Vector3, lookAt:Vector3):CFrame;

	/**
	 * Returns a CFrame with a position comprised of the provided x, y, and z components.
	 */
	@:native("new")
	static function fromXYZ(x:Float, y:Float, z:Float):CFrame;

	/**
	 * Returns a CFrame from position (x, y, z) and quaternion (qX, qY, qZ, qW).
	 */
	@:native("new")
	static function fromPositionAndQuaternion(x:Float, y:Float, z:Float, qX:Float, qY:Float, qZ:Float, qW:Float):CFrame;

	/**
	 * Returns a CFrame from position (x, y, z) with an orientation specified by the rotation matrix.
	 */
	@:native("new")
	static function fromPositionAndMatrix(x:Float, y:Float, z:Float, R00:Float, R01:Float, R02:Float, R10:Float, R11:Float, R12:Float, R20:Float, R21:Float,
		R22:Float):CFrame;

	/**
	 * Returns a CFrame with the position of the first Vector3 and an orientation pointed toward the second.
	 */
	static function lookAt(at:Vector3, lookAt:Vector3, up:Vector3):CFrame;

	/**
	 * Returns a CFrame with the position of the first Vector3 and an orientation directed along the second.
	 */
	static function lookAlong(at:Vector3, direction:Vector3, up:Vector3):CFrame;
	 ...
	 */
	static function fromRotationBetweenVectors(from:Vector3, to:Vector3):CFrame;
	 ...
	 */
	static function fromEulerAngles(rx:Float, ry:Float, rz:Float, order:RotationOrder):CFrame;

	/**
	 * Returns a rotated CFrame from angles rx, ry, and rz in radians using Enum.RotationOrder.XYZ.
	 */
	static function fromEulerAnglesXYZ(rx:Float, ry:Float, rz:Float):CFrame;

	/**
	 * Returns a rotated CFrame from angles rx, ry, and rz in radians using Enum.RotationOrder.YXZ.
	 */
	static function fromEulerAnglesYXZ(rx:Float, ry:Float, rz:Float):CFrame;
	 ...
	 */
	static function fromOrientation(rx:Float, ry:Float, rz:Float):CFrame;

	/**
	 * Returns a rotated CFrame from a unit Vector3 and a rotation in radians.
	 */
	static function fromAxisAngle(v:Vector3, r:Float):CFrame;

	/**
	 * Returns a CFrame from a translation and the columns of a rotation matrix.
	 */
	static function fromMatrix(pos:Vector3, vX:Vector3, vY:Vector3, vZ:Vector3):CFrame;
	 ...
	 */
	function components():Rest<Float>;
